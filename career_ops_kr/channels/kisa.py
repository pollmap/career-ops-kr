"""KISA channel — 한국인터넷진흥원 채용 (kisa.or.kr).

Backend: ``requests``. Scrapes career/recruitment listings.
Legitimacy: T2 (quasi-government, cyber security).
"""
    ...
    def list_jobs(self, query: dict[str, Any] | None = None) -> list[JobRecord]:
        ...
        def _emit(title: str, href: str) -> None:
            ...
            if href_full.startswith("/"):
                href_full = "https://www.kisa.or.kr" + href_full
            if "://" not in href_full:
                return
            ...
            try:
                ...
                self.logger.warning("kisa: skip row: %s", exc)
        ...
        if not results:
            for anchor in soup.find_all("a"):
                ...
                if not text or not href_raw:
                    continue
                if any(kw in text for kw in ("채용", "공고", "모집", "경력", "인턴", "보안", "사이버")):
