"""FSS channel — 금융감독원 채용 (fss.or.kr).

Backend: ``requests``. Scrapes career/recruitment listings.
Legitimacy: T1 (financial regulator).
"""
...
LANDING_URL = "https://www.fss.or.kr/fss/main/sub1Contents.do?menuNo=200600"
USER_AGENT = "career-ops-kr/0.2 (+https://github.com/pollmap/career-ops-kr)"


class FssChannel(BaseChannel):
    """금융감독원 채용 scraper."""
    ...
    def list_jobs(self, query: dict[str, Any] | None = None) -> list[JobRecord]:
        ...
        rows = soup.select("table tr, .board-list tr, .boardList tr, ul.list li, .bbs_list tr, .tb_list tr")
        ...
        def _emit(title: str, href: str) -> None:
            ...
            if href_full.startswith("/"):
                href_full = "https://www.fss.or.kr" + href_full
            if "://" not in href_full:
                return
            ...
            try:
                ...
                self.logger.warning("fss: skip row: %s", exc)
        ...
        if not results:
            for anchor in soup.find_all("a"):
                ...
                if not text or not href_raw:
                    continue
                if any(kw in text for kw in ("채용", "공고", "모집", "경력", "인턴", "금감원")):
