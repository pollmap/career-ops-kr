"""MOFA channel — 외교부 채용 (mofa.go.kr).
...
LANDING_URL = "https://www.mofa.go.kr/www/brd/m_4080/list.do"
USER_AGENT = "career-ops-kr/0.2 (+https://github.com/pollmap/career-ops-kr)"


class MofaChannel(BaseChannel):
    """외교부 채용 scraper."""
    ...
    def list_jobs(self, query: dict[str, Any] | None = None) -> list[JobRecord]:
        ...
        rows = soup.select("table tr, .board-list tr, .boardList tr, ul.list li, .bbs_list tr, .board_list tr")
        ...
        def _emit(title: str, href: str) -> None:
            ...
            if href_full.startswith("/"):
                href_full = "https://www.mofa.go.kr" + href_full
            if "://" not in href_full:
                return
            ...
            try:
                ...
                self.logger.warning("mofa: skip row: %s", exc)
        ...
        if not results:
            for anchor in soup.find_all("a"):
                ...
                if not text or not href_raw:
                    continue
                if any(kw in text for kw in ("채용", "공고", "모집", "경력", "인턴", "외무")):
