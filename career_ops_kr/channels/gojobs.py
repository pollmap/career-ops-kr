"""GoJobs channel — 나라일터 정부일자리포털 (gojobs.go.kr).

Backend: ``requests``. Scrapes government job listings.
Legitimacy: T1 (official government job portal — aggregates all ministries).
"""
...
LANDING_URL = "https://www.gojobs.go.kr/jobList.do"
USER_AGENT = "career-ops-kr/0.2 (+https://github.com/pollmap/career-ops-kr)"


class GoJobsChannel(BaseChannel):
    """나라일터 정부일자리포털 scraper."""
    ...
    def list_jobs(self, query: dict[str, Any] | None = None) -> list[JobRecord]:
        ...
        rows = soup.select(
            ...
            ".job-list li, .recruit-list li, .bbs_list tr, .list-body .item"
        )
        ...
        def _emit(title: str, href: str) -> None:
            ...
            if href_full.startswith("/"):
                href_full = "https://www.gojobs.go.kr" + href_full
            if "://" not in href_full:
                return
            ...
            try:
                ...
                self.logger.warning("gojobs: skip row: %s", exc)
        ...
        if not results:
            for anchor in soup.find_all("a"):
                ...
                if not text or not href_raw:
                    continue
                if any(kw in text for kw in ("채용", "공고", "모집", "경력", "인턴", "공무원", "일자리")):
