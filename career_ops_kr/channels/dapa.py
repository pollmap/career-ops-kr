"""DAPA channel — 방위사업청 채용 (dapa.go.kr).

Backend: ``requests``. Scrapes career/recruitment listings.
Legitimacy: T1 (government agency — defense procurement).
"""
...
LANDING_URL = "https://www.dapa.go.kr/dapa/na/ntt/selectNttList.do?bbsId=326&menuId=678"
USER_AGENT = "career-ops-kr/0.2 (+https://github.com/pollmap/career-ops-kr)"


class DapaChannel(BaseChannel):
    """방위사업청 채용 scraper."""
    ...
    def list_jobs(self, query: dict[str, Any] | None = None) -> list[JobRecord]:
        ...
        def _emit(title: str, href: str) -> None:
            ...
            if href_full.startswith("/"):
                href_full = "https://www.dapa.go.kr" + href_full
            if "://" not in href_full:
                return
            ...
            try:
                ...
                self.logger.warning("dapa: skip row: %s", exc)
        ...
        if not results:
            for anchor in soup.find_all("a"):
                ...
                if not text or not href_raw:
                    continue
                if any(kw in text for kw in ("채용", "공고", "모집", "경력", "인턴", "방위")):
