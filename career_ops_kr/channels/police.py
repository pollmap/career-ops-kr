"""Police channel — 경찰청 채용 (police.go.kr).
...
class PoliceChannel(BaseChannel):
    """경찰청 채용 scraper."""
    ...
    def list_jobs(self, query: dict[str, Any] | None = None) -> list[JobRecord]:
        ...
        rows = soup.select("table tr, .board-list tr, .boardList tr, ul.list li, .bbs-list tr")
        ...
        def _emit(title: str, href: str) -> None:
            ...
            if href_full.startswith("/"):
                href_full = "https://www.police.go.kr" + href_full
            if "://" not in href_full:
                return
            ...
            try:
                ...
                self.logger.warning("police: skip row: %s", exc)
        ...
        if not results:
            for anchor in soup.find_all("a"):
                ...
                if not text or not href_raw:
                    continue
                if any(kw in text for kw in ("채용", "공고", "모집", "경채", "순경", "경력", "사이버")):
