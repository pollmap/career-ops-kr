"""FSC channel — 금융위원회 채용 (fsc.go.kr).

Backend: ``requests``. Scrapes career/recruitment listings.
Legitimacy: T1 (government commission).
"""
...
LANDING_URL = "https://www.fsc.go.kr/no010201"
USER_AGENT = "career-ops-kr/0.2 (+https://github.com/pollmap/career-ops-kr)"


class FscChannel(BaseChannel):
    """금융위원회 채용 scraper."""
    ...
    def list_jobs(self, query: dict[str, Any] | None = None) -> list[JobRecord]:
        ...
        def _emit(title: str, href: str) -> None:
            ...
            if href_full.startswith("/"):
                href_full = "https://www.fsc.go.kr" + href_full
            if "://" not in href_full:
                return
            ...
            try:
                ...
                self.logger.warning("fsc: skip row: %s", exc)
        ...
        if not results:
            for anchor in soup.find_all("a"):
                ...
                if not text or not href_raw:
                    continue
                if any(kw in text for kw in ("채용", "공고", "모집", "경력", "인턴", "금융")):
