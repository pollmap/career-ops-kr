...
공고 요구사항을 1~2주짜리 개인 프로젝트로 역설계한다. 찬희 현재 스킬과의 gap을 채우는 포트폴리오 확장용 스프린트 제안.

## Purpose

- 공고 JD → "이걸 증명할 수 있는 작은 프로젝트" 역설계.
- 결과물은 찬희가 GitHub/블로그에 올릴 수 있는 구체적 스프린트.
...
- `modes/_profile.md` / `config/profile.yml` — 찬희 현재 스킬/보유 자산
- 선택 인자:
...
1. **JD 요구사항 추출** — 공고 원문에서:
   ...
   - 우대 사항 (2차 gap)
   - 직무 일상 업무 (proxy task)
2. **찬희 현재 자산 매핑** (`_profile.md` / `config/profile.yml`):
...
3. **Gap 분석** — JD 요구사항 ∩ 현재 자산 → 부족 영역 도출.
   - 예: 공고가 `Solidity` 요구, 찬희 보유 X → gap.
   - 예: 공고가 `Python+FastAPI`, 찬희 Python 있음 → gap 작음.
...
7. **이력서/자소서 bullet 자동 생성** — STAR 구조:
   ...
   - 찬희가 이력서 Experience 섹션에 바로 추가 가능한 형태.
...
- 별도 게이트 없음 (제안만 생성, 실행은 찬희 결정).
- 단, **over-scoped 프로젝트 경고** — 1인 1~2주로 불가능한 규모면 축소 권장.

## Failure handling

- JD에서 기술 스택 추출 실패 → "공고 원문 부실, 재크롤링 필요" 경고.
- 찬희 자산 정보 부족 → `_profile.md` 보강 요청.
- Gap이 너무 크면 (예: 전혀 새로운 언어 5개) "1~2주 내 불가. 공고 재고 권장" 안내.

## 실데이터 원칙

- 찬희가 실제로 보유하지 않은 스킬을 "보유"로 표시 금지.
- 제안 프로젝트의 결과 수치(커버리지, 성능)는 찬희가 실제로 달성 가능한 범위로만 제안.
